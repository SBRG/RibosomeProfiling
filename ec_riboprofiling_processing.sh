...
do
	...
	do
		...
		bowtie -n 1 -q -p 4 -S ./Indices/$index ${fastq%.*}_trimmed.fastq ${fastq%.*}_${index}_aligned.sam
	done
	
	echo 'Now mapping against all RNA species index'
	bowtie -n 1 -q -p 4 ./Indices/ec_RNA_all_btw ${fastq%.*}_trimmed.fastq --un ${fastq%.*}_${index}_unaligned.fastq > reads.fastq
	rm reads.fastq
	
	echo 'Now mapping unaligned reads to reference genome'
	bowtie -n 1 -q -p 4 -S ./Indices/ec_all_btw ${fastq%.*}_${index}_unaligned.fastq ${fastq%.*}_non_RNA_aligned.sam
